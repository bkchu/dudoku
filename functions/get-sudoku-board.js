...
exports.handler = async function (event, context, callback) {
  ...
  try {
    // use sudoku.js to generate boards
    const root = {}
    sudokujs(root)
    const { sudoku } = root
    const preconvertedBoard = sudoku.generate(difficulty)

    // convert the board from sudoku.js into the structure needed by the app
    const board = _.chunk(
      preconvertedBoard
        .replace(/\./g, "0") // replaces all the . with 0's
        .split("") // turns the string into an array (using nothing as the delimiter)
        .map(char => +char), // turns each char into a number
      9 // chunks the entire array into groups of 9 elements
    )
...
/*
    Sudoku.js - taken from https://www.github.com/robatron/sudoku.js
    ---------
    A Sudoku puzzle generator and solver JavaScript library.
*/
function sudokujs(root) {
